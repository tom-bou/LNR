...
def get_noise_index_refined(train_dataloader,g_head,net, net_id, args, thre = 3, class_num = 100, read = False, store = False, cm = 0, method = 'loge'):
    ...
    if not read and not store:
        return None
    if read:
        if os.path.exists('./output/noises/'+args.id+'_net_'+str(net_id)+'_'+'noise_info.pkl'):
            with open('./output/noises/'+args.id+'_net_'+str(net_id)+'_'+'noise_info.pkl','rb') as j:
    ...
    if store:
        with open('./output/noises/'+args.id+'_net_'+str(net_id)+'_'+'noise_info.pkl', 'wb') as file:
            pickle.dump(noise_info, file)
    print(net_id, noisecnt, comp, label_cnt)
    return noise_info
...
class LocalUpdate(object):
    ...
    def update_weights_gaux(self, net, g_head, g_aux, l_head, epoch, args, mu=1, lr=None, loss_switch=None, isbest = False, net_id = 0, rnd = 0):
        ...
        if rnd >= args.noisecal:
            if isbest:
                read = 0
            else:
                read = 1
            noise_info = get_noise_index_refined(self.ldr_train,copy.deepcopy(g_head),copy.deepcopy(net),net_id,args,
                                        thre = args.thre, class_num = args.num_classes, read =read, store= isbest, cm=rnd)
        epoch_loss = []
        fflag = 0
        for iter in range(epoch):
            batch_loss = []
            ...
            for batch_idx, (index, images, labels) in enumerate(self.ldr_train):
                images, labels = images.to(
                    self.args.device), labels.to(self.args.device)
                if rnd >= args.noisestart:
                    for j, value in enumerate(index):
                        if str(index[j]) in noise_info['noise_flag'].keys():
                            fflag = fflag + 1
                            #print(str(index[j]),labels[j],'->', noise_info['noise_flag'][str(index[j])][0])
                            labels[j] = noise_info['noise_flag'][str(index[j])][0]
                labels = labels.long()
            ...
            epoch_loss.append(sum(batch_loss)/len(batch_loss))
        if rnd > args.noisecal:
            print('net_id:',net_id, ' has noise:', fflag)
    ...
    def update_weights_etf(self, net, g_head, g_aux, l_head, epoch,args, mu=1, lr=None, loss_switch=None, isbest = False, net_id = 0, rnd = 0):
        ...
        if rnd >= args.noisecal:
            if isbest:
                ...
                read = 1
            noise_info = get_noise_index_refined(self.ldr_train,copy.deepcopy(g_head),copy.deepcopy(net),net_id,args,
                                        thre = args.thre, class_num = args.num_classes, read =read, store= isbest, cm=rnd, method='etf')
...
## gain personalzied l_head
    def update_weights_norm_init(self, net, net_id, args, g_aux, g_head, l_head, seed, net_glob, epoch, mu=1, lr=None):
        ...
        net.train()
        # train and update
        #noise_info = get_noise_index_refined(self.ldr_train,g_head,net,net_id,args,
